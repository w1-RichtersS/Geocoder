...
{
    var veld_type_array = new Array('Kies datatype', 'volledig adres', 'straat', 'huisnummer', 'toevoeging', 'postcode', 'woonplaats', 'label');
    ...
    //   $("#div_excel_upload").html(batch_content);

    $("#upload_check_button").click(function()
    {
        var eerste_regel_checked = $("input[name='check_eerste_regel']").is(':checked');
        //        alert( $("input[name='check_eerste_regel']").is(':checked') );

        var all_data = new Array();
        //       alert (($('#check_eerste_regel').attr('checked')));

        //        $("#import_exec").html("<br /><button id=execute_import class=\"more-space\">Geocoding uitvoeren</button>");
   ...
   $("#gridtable tr").each(function(recnr)
    {
        if (recnr > 0)
        {
            ...
            $.each(this.cells, function(i)
            {
                ...
                //            point_feature.attributes.kolom1 = "waarde1";
                //            alert ($(this).html());
